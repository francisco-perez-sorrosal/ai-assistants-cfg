...
5. **Execute the pipeline.** First, fire **GATE 4** per §Phase Gates (the umbrella headline that announces the whole pipeline). Then act as orchestrator — delegate to Praxion subagents via the `Task` tool per `rules/swe/swe-agent-coordination-protocol.md`. For the seed phase, use the **compact Standard-tier** variant. Each delegation is wrapped in its own sub-gate (4a–4e per §Phase Gates) — fire the sub-gate *before* invoking the subagent so the user sees what file(s) will appear (or what in-chat output to expect) before any work runs. Sub-gates honor the same one-way `Run all rest` flag set at any earlier gate, so users who opted out of pauses are not re-prompted.
   ...
   - **5b.** Fire **GATE 4b**, then delegate to `systems-architect` — inline module-shape + dependency-direction summary printed in chat (do NOT create persistent ADRs or `SYSTEMS_PLAN.md` for the seed). Output: in-chat summary only.
   - **5c.** Fire **GATE 4c**, then delegate to `implementation-planner` — 3–5 step decomposition. Output: `.ai-work/<task-slug>/IMPLEMENTATION_PLAN.md` (the canonical ordered step list).
...
| 4b | step 5b (systems-architect) | `Pipeline step 2 of 5: systems-architect. Designs module shape (src/agent/ vs src/web/) and the one-way dependency rule. For this compact seed run, output is an in-chat summary — no persistent SYSTEMS_PLAN.md or ADR. Your first real feature later will produce both. Continue?` |
| 4c | step 5c (implementation-planner) | `Pipeline step 3 of 5: implementation-planner. Decomposes the architect's design into 3–5 atomic, testable steps. Produces .ai-work/<task-slug>/IMPLEMENTATION_PLAN.md — the canonical ordered step list that implementer + test-engineer pull from. Continue?` |
