...
| Rules | Two-channel delivery: 5 core rules + 14 path-scoped rules symlinked into `~/.claude/rules/`; 3 blacklistable always-loaded rules delivered via SessionStart hook `inject_rules.py` based on per-project `.claude/praxion-rules.yaml` config. Manifest-driven taxonomy via auto-generated `rules/_manifest.yaml`. ML-specific rules added under `rules/ml/` (ai-training-onramp v1) | `rules/swe/`, `rules/writing/`, `rules/ml/`, `rules/_manifest.yaml`, `hooks/inject_rules.py`, `scripts/regenerate_rules_manifest.py`, `docs/rules-taxonomy.md` |
