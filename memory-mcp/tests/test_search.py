...
def _write_store(memory_file: Path, entries_by_category: dict) -> None:
    ...
    memories = {cat: {} for cat in ("user", "assistant", "project", "relationships", "tools", "learnings")}
...
class TestRecencyRanking:
        ...
        _write_store(memory_file, {
            "learnings": {
                "old_item": _make_entry(
                    "python pattern old",
                    importance=5, access_count=1, last_accessed=old,
                ),
                "recent_item": _make_entry(
                    "python pattern new",
                    importance=5, access_count=1, last_accessed=recent,
                ),
            },
        })
        store = MemoryStore(memory_file)
    ...
    def test_recency_exponential_decay(self, memory_file: Path):
        now = datetime.now(UTC)
        thirty_days_ago = (now - timedelta(days=RECENCY_DECAY_DAYS)).isoformat().replace("+00:00", "Z")

        _write_store(memory_file, {
            "user": {
                "old_entry": _make_entry(
                    "python topic",
                    importance=5, access_count=1, last_accessed=thirty_days_ago,
                ),
            },
        })
        store = MemoryStore(memory_file)
    ...
    def test_recent_entry_has_higher_recency_signal(self, memory_file: Path):
        ...
        _write_store(memory_file, {
            "user": {
                "recent": _make_entry(
                    "python topic A",
                    importance=5, access_count=3, last_accessed=one_day_ago,
                ),
                "old": _make_entry(
                    "python topic B",
                    importance=5, access_count=3, last_accessed=sixty_days_ago,
                ),
            },
        })
        store = MemoryStore(memory_file)
        result = store.search("python")
        signals_by_key = {
            r["key"]: r["signals"]["recency"] for r in result["results"]
        }
        ...
        _write_store(memory_file, {
            "user": {
                ...
                "python_version": _make_entry(
                    "Currently using python 3.13", importance=5,
                ),
            },
        })
        store = MemoryStore(memory_file)
...
class TestAccessTrackingWithRankedSearch:
    ...
    def test_search_sets_last_accessed(self, store: MemoryStore):
        ...
        # Forget and recreate to reset
        store2.forget("user", "name")
...
class TestCombinedRanking:
    ...
    def test_score_is_weighted_sum_of_signals(self, store: MemoryStore):
        ...
        signals = item["signals"]
        expected_score = sum(
            SEARCH_WEIGHTS[k] * v for k, v in signals.items()
        )
...
class TestScoringEdgeCases:
    ...
    def test_importance_1_gives_minimum_score(self, store: MemoryStore):
        ...
        assert result["results"][0]["signals"]["importance"] == pytest.approx(
            expected, abs=0.01
        )
