...
> **Dual representation of Agents.** Praxion treats agents on two axes: the **authored Markdown family** under `agents/*.md` (a deployable knowledge surface, peer of skills / rules / commands — see the "Agents (authored definitions)" row) and the **spawned-subprocess runtime** that executes one of those definitions inside its own context window (orchestration concern — see the "Agent runtime / Pipeline" row). The two are linked by `knowledge.agents -> orchestration.pipeline "spawns"` in the LikeC4 model. See draft ADR `architecture-c4-dual-agent-representation`.
