...
This decision pairs with `dec-149` (the agent shape), `dec-150` (the four hat-skills the designer reads to make its decisions), and `dec-154` (the active dynamic layered on this default partition — the standing-to-challenge, the architect's obligation-to-re-evaluate, the orchestrator-mediated loop, the user-escalation path). Recorded in `LEARNINGS.md ### Decisions Made` by the implementation-planner.
