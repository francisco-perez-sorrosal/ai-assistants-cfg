...
| **Last verified** | 2026-05-07 (pipeline-dashboard feature Designed: new component **Pipeline Dashboard** added to §3 — Praxion-bundled Streamlit reader with six surfaces (Architecture, Workshops, ADRs, Sentinel, Roadmap, Metrics) reading `.ai-state/` + `.ai-work/<task-slug>/` directly with no new persistence layer; bash ctl + macOS launchd lifecycle mirroring `phoenix-ctl`; per-project sha256-derived port mirroring `chronograph-ctl`; dedicated `~/.praxion-dashboard/venv/` isolation; macOS-only v1 with documented manual-launch fallback for Linux/Windows; soft-deprecates `metrics-viewer.html.tmpl` over one release cycle. Ten draft ADRs under `.ai-state/decisions/drafts/`: process model, port allocation, cross-platform scope, frontmatter parsing, Mermaid deferral, poll interval, dependency isolation, visualization stack, CLAUDE.md placement, metrics-viewer supersession. Implementation deferred to implementation-planner spawn; `docs/architecture.md` not yet updated — Built status pending.); prior: 2026-05-03 (ai-training-onramp feature Designed: third project archetype — ML/AI training — added to the catalog with three new components — Project archetype catalog, ML training subsystem, Neo-cloud abstraction — three new interfaces — `training_job_descriptor`, `TRAINING_RESULTS.md`, `program.md` — three new constraints — descriptor mode-invariance, ML training-loop determinism waiver, mandatory compute-budget declaration. Five draft ADRs under `.ai-state/decisions/drafts/` covering verifier extension, tiered backend strategy, program.md placement, skill scope, and `TRAINING_RESULTS.md` schema ownership. Implementation deferred to implementation-planner spawn.); prior: 2026-04-26 (praxion-first-class feature shipped Built: `hooks/inject_subagent_context.py` + `hooks/inject_process_framing.py` + `hooks/auto_complete_install.py` + `scripts/render_claude_md.py` + §Praxion Process canonical block in `commands/onboard-project.md`/`commands/new-project.md`; three draft ADRs under `.ai-state/decisions/drafts/` awaiting finalize); prior: 2026-04-24 (project-metrics feature shipped Built: `/project-metrics` slash command + `scripts/project_metrics/` package + `docs/metrics/README.md` schema reference + `docs/metrics/index.html` trend-visualization page; five `/project-metrics` draft ADRs awaiting finalize. Tech-debt-integration shipped Built end-to-end: `.ai-state/TECH_DEBT_LEDGER.md` empty artifact + 15-field schema in `rules/swe/agent-intermediate-documents.md` + `scripts/finalize_tech_debt_ledger.py` post-merge dedupe + hook-chain wiring in `scripts/git-post-merge-hook.sh` + verifier Phase 5/5.5 ledger writes + sentinel TD01-TD05 dimension + 5 consumer-contract additions across systems-architect / implementation-planner / implementer / test-engineer / doc-engineer + `## Technical Debt` removed from LEARNINGS template; three draft ADRs under `.ai-state/decisions/drafts/` finalize to stable `dec-NNN` at merge-to-main) |
...
|-----------|---------------|--------|--------------------------|
| Skills | Domain expertise delivered via progressive disclosure (metadata, body, references). 35 skills Built as of 2026-04-16; Phase 4 added `llm-prompt-engineering`. ai-training-onramp v1 added: `ml-training`, `llm-training-eval`, `neo-cloud-abstraction`, `experiment-tracking` (see ML training subsystem row). | Built | `skills/*/SKILL.md`, `skills/*/references/`, `skills/llm-prompt-engineering/`, `skills/ml-training/`, `skills/llm-training-eval/`, `skills/neo-cloud-abstraction/`, `skills/experiment-tracking/` |
...
| ML training subsystem (v1) | The skill set, rules, and command extensions enabling Praxion to manage ML/AI pre-training projects. **Skills (new)**: `ml-training` (pre-training conventions, framework patterns, distributed training), `llm-training-eval` (eval methodology, val_bpb/perplexity metrics, `TRAINING_RESULTS.md` schema), `experiment-tracking` (standalone skill — MLflow / W&B / Aim conventions for ML experiment lineage; conceptually distinct from app observability, with different time horizons, data shapes, decision models, and toolchains, per Q3 override). **Skill extensions**: `gpu-compute-budgeting` reference under `deployment`; `ml-experiment-ci` reference under `cicd`. **Rules (new)**: `rules/ml/eval-driven-verification.md` (always-loaded, ~30 lines), `rules/ml/gpu-budget-conventions.md` (always-loaded, ~30 lines), `rules/ml/experiment-tracking-conventions.md` (path-scoped to `runs/**`, `experiments/**`, `program.md`). **Rule extensions**: scoped determinism waiver in `rules/swe/testing-conventions.md`; experiment-mode addendum in `rules/swe/vcs/git-conventions.md`. **Agent extensions**: `verifier` Phase 3 eval-aware sub-branch (reads `TRAINING_RESULTS.md`); `cicd-engineer` ML CI satellite reference. **Commands (new)**: `/run-experiment`, `/check-experiment`. **Command extension**: `/onboard-project` Phase 8c (ML detection + scaffold). **Artifact category (new)**: `program.md` as project-local meta-prompt sibling of CLAUDE.md (`dec-115`). See `dec-119`, `dec-117`, `dec-116`. | Built | `skills/ml-training/`, `skills/llm-training-eval/`, `skills/experiment-tracking/`, `skills/deployment/references/gpu-compute-budgeting.md`, `skills/cicd/references/ml-experiment-ci.md`, `rules/ml/eval-driven-verification.md`, `rules/ml/gpu-budget-conventions.md`, `rules/ml/experiment-tracking-conventions.md`, `commands/run-experiment.md`, `commands/check-experiment.md`, `commands/onboard-project.md` (Phase 8c), `agents/verifier.md` (Phase 3a), `agents/cicd-engineer.md` (ML detection step) |
| Neo-cloud abstraction | Generic Praxion-native abstraction surface for ML training compute lifecycle. Defines `training_job_descriptor` schema (mode-invariant; modes A/B/C share one schema) and 8 lifecycle operations (create, start, status, log_stream, cancel, artifact_fetch, list, pricing_query). **Tiered backend strategy**: local default (`subprocess.run` semantics, modes A/B), SkyPilot 0.12.1 default-remote (mode C, covers 20+ providers), opt-in direct adapters (RunPod via `@runpod/mcp-server` 1.1.0 is the v1 reference specialization; Lambda/Crusoe/CoreWeave direct are v2). The descriptor does not contain a `mode:` field — backend is inferred from project config. The local backend's `pricing_query` returning `0.0` is the canonical "no-op or trivially-implementable" pattern proving the abstraction's correctness. See `dec-118`. | Built | `skills/neo-cloud-abstraction/SKILL.md`, `skills/neo-cloud-abstraction/references/local-backend.md`, `skills/neo-cloud-abstraction/references/skypilot-backend.md`, `skills/neo-cloud-abstraction/references/runpod-direct-adapter.md` |
| Pipeline Dashboard | Praxion-bundled Streamlit dashboard providing a per-project visual entry point that reads `.ai-state/` and `.ai-work/<task-slug>/` directly with **no new persistence layer**. Six surfaces (wide v1): Architecture (LikeC4 SVG embed + AaC fence-aware Components table), Workshops (in-flight pipeline supervision with `st.fragment(run_every=15s)` mtime-keyed live refresh; reflects transient `.ai-work/` state and degrades to empty when cleaned up), ADRs (browser over finalized + draft fragment files with frontmatter parsing, status/tag filters, supersession lineage DAG), Sentinel (latest report scorecard + `SENTINEL_LOG.md` health-grade sparkline), Roadmap (`<project-root>/ROADMAP.md` reader with empty-state fallback per dec-092), Metrics (15-field aggregate trend + KPI grid superseding `metrics-viewer.html.tmpl` per dec-draft-4b63cccd). Educational layer: every artifact card carries a "What is this?" affordance (popover/expander) linking to the producing skill/rule/agent — the dashboard teaches Praxion to its operator. Process model: standalone bash ctl + macOS launchd plist (mirrors `phoenix-ctl`); per-project sha256-derived port 8501–9500 (mirrors `chronograph-ctl`); dedicated `~/.praxion-dashboard/venv/` isolation. Visualization: `st.graphviz_chart` (DAGs, bundled) + `plotly` (heatmaps, transitive Streamlit dep) + `pyvis` (interactive fallback). Mermaid rendering deferred to v2 (no in-pipeline Mermaid emitted today). macOS-only daemon v1; Linux/Windows manual-launch path documented; systemd deferred to v2. Soft-deprecates `claude/aac-templates/metrics-viewer.html.tmpl` over one release cycle. Read-only by design; v1 never writes to `.ai-state/` or `.ai-work/`. See dec-draft-bcaea27e (process model), dec-draft-dd356bb0 (port), dec-draft-df080384 (cross-platform scope), dec-draft-1c4350fd (frontmatter parsing), dec-draft-d57dc712 (Mermaid deferral), dec-draft-273788b7 (poll interval), dec-draft-78f800c9 (dependency isolation), dec-draft-f96cdeec (visualization stack), dec-draft-0c81225f (CLAUDE.md placement), dec-draft-4b63cccd (metrics-viewer supersession). | Built | `streamlit_app/app.py`, `streamlit_app/launcher.py`, `streamlit_app/data/{discovery,parsers,cache}.py`, `streamlit_app/pages/{architecture,workshops,adrs,sentinel,roadmap,metrics}.py`, `streamlit_app/widgets/{artifact_card,educational,graph,empty_state}.py`, `streamlit_app/config.py`, `streamlit_app/requirements.txt`, `streamlit_app/tests/`, `scripts/praxion-dashboard`, `commands/dashboard.md` |
