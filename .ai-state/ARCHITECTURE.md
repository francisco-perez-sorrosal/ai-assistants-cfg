...
| Pipeline Dashboard | Praxion-bundled Streamlit dashboard providing a per-project visual entry point that reads `.ai-state/` and `.ai-work/<task-slug>/` directly with **no new persistence layer**. Six surfaces (wide v1): Architecture (LikeC4 SVG embed + AaC fence-aware Components table), Workshops (in-flight pipeline supervision with `st.fragment(run_every=15s)` mtime-keyed live refresh; reflects transient `.ai-work/` state and degrades to empty when cleaned up), ADRs (browser over finalized + draft fragment files with frontmatter parsing, status/tag filters, supersession lineage DAG), Sentinel (latest report scorecard + `SENTINEL_LOG.md` health-grade sparkline), Roadmap (`<project-root>/ROADMAP.md` reader with empty-state fallback per dec-092), Metrics (15-field aggregate trend + KPI grid superseding `metrics-viewer.html.tmpl` per dec-129). Educational layer: every artifact card carries a "What is this?" affordance (popover/expander) linking to the producing skill/rule/agent — the dashboard teaches Praxion to its operator. Process model: standalone bash ctl + macOS launchd plist (mirrors `phoenix-ctl`); per-project sha256-derived port 8501–9500 (mirrors `chronograph-ctl`); dedicated `~/.praxion-dashboard/venv/` isolation. Visualization: `st.graphviz_chart` (DAGs, bundled) + `plotly` (heatmaps, transitive Streamlit dep) + `pyvis` (interactive fallback). Mermaid rendering deferred to v2 (no in-pipeline Mermaid emitted today). macOS-only daemon v1; Linux/Windows manual-launch path documented; systemd deferred to v2. Soft-deprecates `claude/aac-templates/metrics-viewer.html.tmpl` over one release cycle. Read-only by design; v1 never writes to `.ai-state/` or `.ai-work/`. See dec-128 (process model), dec-127 (port), dec-122 (cross-platform scope), dec-124 (frontmatter parsing), dec-125 (Mermaid deferral), dec-126 (poll interval), dec-123 (dependency isolation), dec-130 (visualization stack), dec-121 (CLAUDE.md placement), dec-129 (metrics-viewer supersession). | Built | `streamlit_app/app.py`, `streamlit_app/launcher.py`, `streamlit_app/data/{discovery,parsers,cache}.py`, `streamlit_app/pages/{architecture,workshops,adrs,sentinel,roadmap,metrics}.py`, `streamlit_app/widgets/{artifact_card,educational,graph,empty_state}.py`, `streamlit_app/config.py`, `streamlit_app/requirements.txt`, `streamlit_app/tests/`, `scripts/praxion-dashboard`, `commands/dashboard.md` |
