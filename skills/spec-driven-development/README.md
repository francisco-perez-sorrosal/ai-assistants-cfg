...
| File | Purpose |
|------|---------|
| `SKILL.md` | Core skill: complexity triage, spec format, REQ ID conventions, traceability threading, decision documentation, spec archival |
| `README.md` | This file -- overview and usage guide |
| `references/spec-format-guide.md` | Full spec format examples, EARS/GWT comparison, traceability matrix template, persistent spec template |
...
3. **Thread IDs**: the implementation-planner threads REQ IDs into test steps
4. **Name tests**: prefix test names with `req{NN}_` for traceability
5. **Verify**: the verifier produces a traceability matrix linking requirements to tests
6. **Archive**: the planner archives the spec to `.ai-state/specs/` at end-of-feature
