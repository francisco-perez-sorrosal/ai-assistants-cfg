...
| **[sentrux](sentrux/)** | Sentrux structural quality sensor integration — external Rust-native marketplace plugin scoring code structure across 5 root-cause metrics via 15 MCP tools. Covers the session-bracketing pattern, the 0-10000 quality score, plugin/binary metric drift, the verifier's advisory Phase 9b, and the open policy decisions deferred to the systems-architect. | Bracketing agent coding sessions with `session_start` / `session_end`, interpreting `sentrux scan` / `health` / `architecture` output, authoring `.sentrux/rules.toml`, deciding whether to elevate sentrux signals from advisory to gating. |
